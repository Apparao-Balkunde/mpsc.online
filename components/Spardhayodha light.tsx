...
export const SpardhaYodha: React.FC<Props> = ({ onBack }) => {
  ...
  // RESULT
  return (
    <div style={{ ...base, display:'flex', flexDirection:'column', alignItems:'center', justifyContent:'center', padding:'24px 16px', position:'relative', overflow:'hidden' }}>
      <style>{CSS}</style>
      ...
      <div style={{ background:'#fff', border:'1px solid rgba(99,79,192,0.10)', borderRadius:32, padding:'32px 24px', maxWidth:420, width:'100%', animation:'sy-result-in 0.6s cubic-bezier(.34,1.56,.64,1)', position:'relative', overflow:'hidden', zIndex:2, boxShadow:'0 12px 48px rgba(99,79,192,0.12)' }}>
        ...
        <div style={{ textAlign:'center', marginBottom:20 }}>
          <div style={{ fontSize:72, animation:'sy-trophy 0.8s cubic-bezier(.34,1.56,.64,1) 0.2s both', display:'inline-block' }}>
            {rank.emoji}
          </div>
        ...
        <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gap:8, marginBottom:18 }}>
          {[
            ...
            { l:'चुकीचे',   v:TOTAL_Q-score,      c:'#DC2626', bg:'rgba(220,38,38,0.08)', border:'rgba(220,38,38,0.2)' },
            { l:'Max Combo', v:`${maxCombo}x 🔥`,  c:'#EA580C', bg:'rgba(249,115,22,0.08)',border:'rgba(249,115,22,0.2)' },
          ].map(({ l,v,c,bg,border }) => (
        ...
        {/* WA preview */}
        <div style={{ background:'#F0FDF4', border:'1px solid #86EFAC', borderRadius:14, padding:'12px 14px', marginBottom:14, fontSize:12, color:'#166534', fontWeight:600, lineHeight:1.65, textAlign:'center' }}>
