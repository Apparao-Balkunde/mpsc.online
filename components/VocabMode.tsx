...
import { getProgress, toggleVocabBookmark } from '../services/progress';
// Fixed: Removed SwapIcon which is not exported by lucide-react and is unused
import { BookA, Loader2, ArrowLeft, RotateCw, Volume2, Bookmark, Database } from 'lucide-react';
    ...
    if (lower.startsWith('vs:') || lower.includes('pair:')) {
      return (
        <div key={text} className="w-full mt-2 p-3 rounded-xl border flex flex-col gap-1 bg-amber-50 text-amber-900 border-amber-400 font-bold shadow-sm">
            <div className="flex items-center text-[10px] uppercase font-black text-amber-600">MPSC TRAP</div>
        ...
        <div className="w-full md:w-80 bg-white rounded-xl shadow-md border border-slate-100 overflow-hidden shrink-0 sticky top-24">
            ...
            <div className="p-4 space-y-6">
                ...
                <div>
                    ...
                    <select value={category} onChange={(e) => setCategory(e.target.value as VocabCategory)} className="w-full p-2.5 border border-slate-200 rounded-lg text-sm">
        ...
        <div className="flex-1 min-w-0">
             ...
             {status === 'success' && words.length > 0 && (
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    {words.map((item, idx) => (
                        <div key={idx} className="bg-white rounded-xl shadow-sm border border-slate-200 p-6 flex flex-col group hover:shadow-lg transition-all relative">
                            <div className="flex justify-between items-start mb-3">
                                ...
                                <div className="flex items-center gap-2">
                                  <button onClick={() => handleToggleBookmark(item)} className={`${bookmarks.includes(item.word) ? 'text-pink-500' : 'text-slate-300'} hover:text-pink-600 p-1`}>
                                    <Bookmark size={20} fill={bookmarks.includes(item.word) ? 'currentColor' : 'none'} />
                                  </button>
                                  <button onClick={() => handlePlayAudio(item.word, idx)} className="text-slate-400 hover:text-indigo-600 p-1">
                                    {playingIndex === idx ? <Loader2 size={20} className="animate-spin"/> : <Volume2 size={20} />}
                                  </button>
                                </div>
                            </div>
