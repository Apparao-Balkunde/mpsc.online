        ...
        <div className="p-10 bg-indigo-700 text-white relative">
          ...
          <div className="flex flex-col md:flex-row justify-between items-start gap-8 relative z-10">
             <div>
                ...
                <p className="text-indigo-100 text-lg font-medium">Simple word analysis for fast revision.</p>
             </div>
                ...
                <div className="space-y-10">
                    <div className="flex flex-col md:flex-row justify-between items-center gap-6 bg-white p-8 rounded-[2rem] border border-slate-100 shadow-sm">
                        <div className="flex items-center gap-5">
                            ...
                            <div>
                                <h4 className="font-black text-slate-900 text-lg">Question List</h4>
                                <p className="text-xs font-bold text-slate-400 uppercase tracking-widest">{subject} Analysis</p>
                            </div>
                        </div>
                    </div>
                            ...
                            <div key={idx} className="bg-white rounded-[3rem] shadow-2xl border border-slate-200 p-12 hover:border-indigo-400 transition-all relative group overflow-hidden">
                                    ...
                                    <div className="flex gap-8">
                                        ...
                                        <div>
                                            <div className="flex items-center gap-3 mb-2">
                                                <span className="text-[10px] font-black text-indigo-500 uppercase tracking-widest bg-indigo-50 px-4 py-1.5 rounded-full border border-indigo-100">{selectedYear} Paper</span>
                                            </div>
                                ...
                                <div className="ml-0 md:ml-22 flex flex-col items-start gap-6">
                                    <button onClick={() => toggleReveal(idx)} className={`px-12 py-5 rounded-[1.5rem] text-sm font-black uppercase tracking-widest shadow-2xl transition-all flex items-center gap-3 ${revealedAnswers.includes(idx) ? 'bg-slate-900 text-white' : 'bg-indigo-600 text-white hover:bg-indigo-700'}`}>
                                        {revealedAnswers.includes(idx) ? 'Hide Info' : 'Show Simple Summary'}
                                        {revealedAnswers.includes(idx) ? <ArrowLeft size={18} /> : <Eye size={18} />}
                                    </button>
                                    
                                    {revealedAnswers.includes(idx) && (
                                        <div className="w-full mt-6 space-y-6 animate-in slide-in-from-top-6 duration-700">
                                            {q.mnemonic && (
                                                <div className="bg-amber-100 p-8 rounded-[2rem] border-2 border-amber-200 flex items-start gap-6">
                                                    <div className="bg-amber-500 text-white p-3 rounded-2xl shadow-xl"><BrainCircuit size={28} /></div>
                                                    <div>
                                                        <h5 className="text-[10px] font-black text-amber-700 uppercase tracking-widest mb-1">Trick</h5>
                                                        <p className="text-xl font-black text-amber-950 italic">"{q.mnemonic}"</p>
                                                    </div>
                                                </div>
                                            )}
                                            
                                            <div className="bg-slate-950 p-10 rounded-[2.5rem] border border-slate-800 text-slate-100 relative shadow-2xl">
                                                <div className="flex items-center gap-4 mb-8 border-b border-slate-800 pb-8">
                                                    <Zap className="text-yellow-400" size={24} />
                                                    <h4 className="text-sm font-black text-slate-400 uppercase tracking-widest">Quick Study Note</h4>
                                                </div>
                                                <div className="prose prose-invert prose-p:leading-relaxed prose-p:text-slate-300 max-w-none text-lg font-medium">
