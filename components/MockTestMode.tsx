...
import { ShieldCheck, Timer, ArrowLeft, ArrowRight, CheckCircle2, AlertCircle, Loader2, Save, Send, Eye, Copy, Check, Settings2, SlidersHorizontal, LayoutGrid, RotateCcw, Zap, Database, Cloud, History, Play, Clock, ListFilter, Info, BookOpen, GraduationCap, BrainCircuit } from 'lucide-react';
  ...
  if (status === 'error') {
    return (
      <div className="max-w-4xl mx-auto p-6 text-center">
        ...
        <button onClick={() => setStatus('idle')} className="bg-indigo-600 text-white px-6 py-3 rounded-xl font-bold">Try Again</button>
      </div>
    );
  }
    ...
    return (
      <div className="max-w-4xl mx-auto p-6 space-y-8">
        ...
        <div className="bg-white rounded-[2rem] shadow-2xl overflow-hidden border border-slate-100">
            <div className="p-12 bg-indigo-700 text-white text-center">
                ...
                <p className="text-indigo-100 italic">"Study deep, score high with AI Analysis"</p>
            </div>
            <div className="p-10 space-y-8">
                <div className="grid md:grid-cols-2 gap-6">
                    <button onClick={() => setExamType('RAJYASEVA')} className={`p-6 rounded-2xl border-2 text-left transition-all ${examType === 'RAJYASEVA' ? 'border-indigo-600 bg-indigo-50 shadow-md' : 'border-slate-100 bg-slate-50'}`}>
                        ...
                        <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Prelims Analysis</p>
                    </button>
                    <button onClick={() => setExamType('GROUP_B')} className={`p-6 rounded-2xl border-2 text-left transition-all ${examType === 'GROUP_B' ? 'border-indigo-600 bg-indigo-50 shadow-md' : 'border-slate-100 bg-slate-50'}`}>
                        ...
                        <p className="text-xs text-slate-500 font-bold uppercase tracking-wider">Group B & C</p>
                    </button>
                </div>
                <div className="grid grid-cols-2 gap-4">
                    <button onClick={() => setTestSource('LOCAL')} className={`p-4 rounded-2xl border-2 font-black transition-all ${testSource === 'LOCAL' ? 'bg-emerald-600 border-emerald-600 text-white' : 'bg-white border-slate-200 text-slate-500'}`}>Offline Bank</button>
                    <button onClick={() => setTestSource('AI')} className={`p-4 rounded-2xl border-2 font-black transition-all ${testSource === 'AI' ? 'bg-indigo-600 border-indigo-600 text-white' : 'bg-white border-slate-200 text-slate-500'}`}>AI Generator</button>
                </div>
                <button onClick={() => startTest()} className="w-full bg-slate-900 text-white py-6 rounded-2xl font-black text-2xl hover:bg-black shadow-2xl transition-all">START PAPER</button>
            </div>
        </div>
      </div>
    );
  ...
  if (status === 'loading') {
    return (
      <div className="max-w-4xl mx-auto p-20 text-center">
        ...
        <h2 className="text-2xl font-black text-slate-800">Generating Interactive Paper...</h2>
        ...
        <div className="bg-white rounded-2xl shadow-md border border-slate-100 overflow-hidden sticky top-24 z-30">
          <div className="px-6 py-4 bg-slate-900 text-white flex justify-between items-center">
             <div className="flex items-center gap-3">
               <span className={`px-2 py-0.5 rounded text-[10px] font-black uppercase ${fromCache ? 'bg-emerald-600' : 'bg-indigo-600'}`}>{fromCache ? 'Static' : 'AI Mode'}</span>
               <h3 className="font-bold">{isFinished ? 'Detailed Analysis' : 'Live Exam'}</h3>
             </div>
        ...
        {isFinished ? (
          <div className="space-y-12 pb-24 animate-in fade-in duration-500">
            <div className="bg-white rounded-[2.5rem] shadow-2xl p-12 text-center border-4 border-indigo-100">
              <h1 className="text-5xl font-black text-indigo-950 mb-6">Final Result</h1>
              <div className="grid grid-cols-3 gap-8 mb-12">
                 <div className="bg-indigo-50 p-8 rounded-3xl border border-indigo-100">
                    <div className="text-[10px] font-black text-indigo-400 uppercase mb-1">Total Qs</div>
                 ...
                 <div className="bg-slate-50 p-8 rounded-3xl border border-slate-200">
                    <div className="text-[10px] font-black text-slate-400 uppercase mb-1">Percentage</div>
                    <div className="text-5xl font-black text-slate-800">{Math.round((getScore() / Math.max(1, questions.length)) * 100)}%</div>
                 </div>
              </div>
              <button onClick={() => setStatus('idle')} className="bg-indigo-600 text-white px-16 py-5 rounded-2xl font-black text-xl shadow-xl hover:bg-indigo-700 transition-all">TRY NEW SET</button>
            </div>
            ...
            <div className="space-y-12">
               <div className="flex items-center gap-4 bg-white p-8 rounded-3xl border border-slate-200 shadow-sm">
                  ...
                  <div>
                    ...
                    <p className="text-slate-500 font-medium">प्रत्येक प्रश्नाचे सखोल विश्लेषण आणि लक्षात ठेवण्याच्या क्लृप्त्या खालीलप्रमाणे आहेत.</p>
                  </div>
               </div>
               ...
               {filteredQuestions.map((q, filteredIdx) => {
                 ...
                 <div key={originalIdx} className="bg-white rounded-[3rem] shadow-2xl p-10 border border-slate-200 relative overflow-hidden group hover:border-indigo-400 transition-all">
                    {isCorrect ? <div className="absolute top-0 right-0 p-5 bg-emerald-100 text-emerald-600 font-black text-xs rounded-bl-3xl uppercase tracking-widest">Perfect Score</div> : <div className="absolute top-0 right-0 p-5 bg-red-100 text-red-600 font-black text-xs rounded-bl-3xl uppercase tracking-widest">Mistake Found</div>}
                    ...
                    <div className="ml-0 md:ml-16 space-y-6">
                        {q.mnemonic && (
                            <div className="bg-amber-100 p-8 rounded-[2.5rem] border-2 border-amber-200 flex items-start gap-5 animate-in zoom-in-95 duration-500">
                                <div className="bg-amber-500 text-white p-3 rounded-2xl shadow-lg"><BrainCircuit size={28} /></div>
                                <div>
                                    <h5 className="text-xs font-black text-amber-700 uppercase tracking-widest mb-1">Mnemonic Master (लक्षात ठेवण्याची ट्रिक)</h5>
                        ...
                        <div className="bg-slate-900 p-12 rounded-[3rem] text-white shadow-2xl relative">
                            ...
                            <div className="flex items-center gap-3 mb-8 border-b border-slate-700 pb-8">
                                ...
                                <div>
                                    <h5 className="text-sm font-black text-slate-400 uppercase tracking-widest">Academic Deep Dive</h5>
                                    <p className="text-xs text-yellow-400/80 font-bold uppercase tracking-tighter italic">Expert teaching note</p>
                                </div>
                            </div>
                            <div className="text-lg text-slate-200 leading-loose prose prose-invert prose-headings:text-indigo-400 prose-headings:font-black prose-p:my-4 prose-ul:my-4 max-w-none font-medium">
                                <ReactMarkdown>{q.explanation}</ReactMarkdown>
                            </div>
                            <div className="mt-12 pt-8 border-t border-slate-800 flex items-center justify-between opacity-30">
                                <span className="text-[10px] font-black uppercase tracking-widest">MPSC Sarathi Academic Library</span>
                                <Info size={20} />
                            </div>
                        </div>
                    </div>
          ...
          <div className="bg-white rounded-[3rem] shadow-2xl p-12 border border-slate-100 min-h-[600px] flex flex-col">
            {filteredQuestions.length === 0 ? (
               <div className="flex-1 flex flex-col items-center justify-center text-center p-10 opacity-30"><ListFilter size={100} className="mb-6 text-slate-300" /><h3 className="text-2xl font-black text-slate-500">No questions in this filter.</h3></div>
            ) : (
               <>
                <div className="flex justify-between items-start mb-12">
                   <div className="flex gap-6">
                     <div className="flex flex-col items-center gap-2 shrink-0">
                        ...
                        <span className="text-[10px] font-black text-slate-400 uppercase tracking-tighter">Current Q</span>
                     </div>
                     <div className="pt-2">
                ...
                <div className="space-y-4 mb-12 ml-0 md:ml-20 flex-1">
                   {filteredQuestions[currentIdx]?.options.map((opt, oIdx) => {
                     ...
                     <button key={oIdx} onClick={() => { const n = [...userAnswers]; n[originalIdx] = oIdx; setUserAnswers(n); }} className={`w-full text-left p-8 rounded-3xl border-2 flex items-center justify-between group transition-all ${userAnswers[originalIdx] === oIdx ? 'border-indigo-600 bg-indigo-50 text-indigo-900 font-bold shadow-xl scale-[1.02]' : 'border-slate-100 hover:border-slate-300 text-slate-700'}`}>
                        ...
                        {userAnswers[originalIdx] === oIdx && <CheckCircle2 size={32} className="text-indigo-600" />}
                     </button>
                   )})}
                </div>
                <div className="flex justify-between items-center pt-12 border-t border-slate-100 mt-auto">
                   <button disabled={currentIdx === 0} onClick={() => setCurrentIdx(prev => prev - 1)} className="flex items-center gap-3 font-black text-slate-400 hover:text-indigo-600 disabled:opacity-20 uppercase text-sm tracking-widest"><ArrowLeft size={24} /> Previous</button>
                   <div className="text-sm font-black text-slate-400 uppercase tracking-widest">Question {currentIdx + 1} of {filteredQuestions.length}</div>
                   {currentIdx === filteredQuestions.length - 1 ? (
                     <button onClick={finishTest} className="bg-emerald-600 text-white px-14 py-5 rounded-2xl font-black text-xl shadow-2xl flex items-center gap-3 hover:bg-emerald-700 active:scale-95 transition-all">FINISH <Send size={24} /></button>
                   ) : (
      ...
      {!isFinished && (
        <div className="w-full md:w-80 shrink-0">
          <div className="bg-white rounded-[2rem] shadow-xl border border-slate-100 p-8 sticky top-24">
             <h4 className="font-black text-slate-900 text-xs uppercase tracking-widest mb-8 border-b border-slate-100 pb-4">Exam Navigator</h4>
             <div className="grid grid-cols-5 gap-3 max-h-[500px] overflow-y-auto pr-2 custom-scrollbar">
                {filteredQuestions.map((q, i) => {
                  ...
                  return <button key={i} onClick={() => setCurrentIdx(i)} className={`h-12 rounded-2xl text-xs font-black transition-all ${currentIdx === i ? 'ring-4 ring-indigo-200 scale-110 z-10' : ''} ${userAnswers[oIdx] !== -1 ? 'bg-indigo-600 text-white shadow-lg' : 'bg-slate-100 text-slate-400 hover:bg-slate-200'}`}>{oIdx + 1}</button>
                })}
             </div>
